...
class Trader:
    ...
    def run(self, state: State):
        ...
        # ULTRA AGGRESSIVE - sometimes cross spread!
        if mr_signal >= 2 and pos < 15:
            # Very cheap AND room to buy - HIT THE ASK
            our_bid = best_ask  # CROSS!
            our_ask = best_ask + 3
            buy_size = 30
            sell_size = 0
        elif mr_signal <= -2 and pos > -15:
            # Very rich AND room to sell - HIT THE BID
            our_ask = best_bid  # CROSS!
            our_bid = best_bid - 3
            sell_size = 30
            buy_size = 0
        elif spread >= 6:
            ...
            # Standard spread - go 2 ticks inside!
            our_bid = best_bid + 2
            our_ask = best_ask - 2
        else:
            # Tight spread - go inside anyway
            our_bid = best_bid + 1
            our_ask = best_ask - 1
